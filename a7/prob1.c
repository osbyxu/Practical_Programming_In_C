...
int find_index(nodekey key, p_tnode pnode) {
	...
	while (L <= R) {
		icmp = (L+R)/2;
		if(key_compare(key,pnode->keys[icmp])==0){
			return -(icmp+1);
		}
		if(key_compare(key,pnode->keys[icmp])){
			L = icmp+1;
			ibetween = L;
		}
		if(key_compare(key,pnode->keys[icmp])==-1){
			R = icmp-1;
			ibetween = R+1;
		}
	}
...
nodevalue * find_value(const nodekey key) {
	 ...
	 */
	while(pleaf!=NULL){
		ichild = find_index(pleaf->keys,pleaf);
		if(ichild<0){
			return pleaf->values[-(ichild+1)];
		}
		pleaf=pleaf->children[ichild];
